...
class SnapchatDownloader(BaseDownloader):
    def fetch_media(self, url: str) -> dict:
        ...
        try:
            resp = self._request("POST", "https://solyptube.com/findsnapchatvideo", headers=headers, json_data={"url": url})
            return resp.json()
        except Exception as e:
            raise RuntimeError(f"Failed to fetch Snapchat data: {e}")

    def download(self, url: str, output_dir: str = "downloads") -> list:
        info = self.fetch_media(url)
        
        # Check standard spotlight/solyptube API response keys
        download_url = info.get("download_url") or info.get("url") or info.get("video_url") or info.get("video")
