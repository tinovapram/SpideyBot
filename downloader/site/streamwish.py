"""StreamWish / Playnixes downloader — packed JS → HLS playlist → yt-dlp."""
...
_DOMAIN_RE = re.compile(
    r"streamwish\.\w+|playnixes\.\w+|swishsrv\.\w+|flaswish\.\w+|"
    r"wishembed\.\w+|embedwish\.\w+|awish\.\w+|filelions\.\w+"
)


class StreamWishDownloader(BaseDownloader):
    """Download from StreamWish / Playnixes and mirrors."""
    ...
    def download(self, url: str, output_dir: str = "downloads") -> list:
        ...
        # Ensure we're hitting /e/{vid} for the embed page
        ...
        if "File is no longer available" in html or "file was deleted" in html.lower():
            raise ValueError("File was deleted on StreamWish")
        ...
        if not packed_match:
            raise ValueError("Could not find packed JS in StreamWish embed")
        ...
        if not selected:
            raise ValueError("No HLS stream found in StreamWish embed")

        # Download HLS via yt-dlp (handles TS segments + reassembly)
